...
class KeyboardControl(object):
    ...
    def __init__(self, role_name, hud, node):
        ...
        self.AEB_obstacle_publisher = self.node.new_publisher(Bool,"/AEB_Obstacle", qos_profile=fast_qos)
    ...
    def callback_(self,ros_point_cloud):
        ...
        for x in int_data:
            ...
            if(x[0] < 6.5 and x[0] > 0.5 and x[1] < 0.5 and x[1]> -0.5 and x[2] < 1 and x[2]> -1.3):
                xyz = np.append(xyz,[[x[0],x[1],x[2]]], axis = 0)
                if self._control.reverse == False:
                    #self.node.logwarn("xxxxxxxxxxxxxxx: {}".format(x[0]))
                    #self.node.logwarn("xxxxxxxxxxxxxxx1: {}".format(x[1]))
                    #self.node.logwarn("xxxxxxxxxxxxxxx2: {}".format(x[2]))
                    self._control.brake = 1
                    self.AEB_activated = True
                    local_AEB_activated = True
                    self.AEB_obstacle_publisher.publish(True)
                    break
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == K_q:
                    ...
                    plt.xlabel('X')
                    plt.ylabel('Y')
                    plt.title('Odometry Diagram')
                    plt.grid(True)
                    plt.pause(0.1)
                    plt.show()
                elif event.key == K_j:
                    self.AutoParking_activated = True
    ...
    def _on_new_carla_frame(self, data):
        ...
        if not self._autopilot_enabled and self.vehicle_control_manual_override:
            try:
                ...
                if self.AutoParking_activated == True:
                    if(self.current_step == 1):
                        ...
                        if self.hud.x > 47:
                            ...
                            self.cnt = self.cnt + 1
                            self._control.brake = 0.1*self.cnt
                            if(self.cnt == 10):
                                self.current_step = 2 
                                self.cnt = 0
                    ...
                    if(self.current_step == 2):
                        print("S2")                                   
                        if self.steer_cache < 0.5:
                            print(self._control.steer)
                            self.steer_cache =self.steer_cache  + 0.05                           
                        else:
                            self.cnt = self.cnt + 1
                            if(self.cnt == 10):
                                self.current_step = 3 
                                self.cnt = 0
                        self._control.steer = self.steer_cache


                    if(self.current_step == 3):  
                        print("S3",self.hud.yaw)   
                        self._control.steer = self.steer_cache
                        self._control.gear = - 1
                        self._control.throttle = 0.5
                        if(self.hud.yaw > -90) and (self.hud.yaw < 0) :
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        ...
        self._control.brake = 1.0 if keys[K_DOWN] or keys[K_s] or (self.AEB_activated and self._control.reverse == False) else 0.0
...
class HUD(object):
    ...
    def odometry_updated(self, data):
        ...
        #x = self.x - 2*math.cos(yaw)
        #y = self.y - 2*math.sin(yaw)
