...
class KeyboardControl(object):
    ...
    def __init__(self, role_name, hud, node):
        ...
        self.carla_status_subscriber = self.node.new_subscription(
            ...
            qos_profile=10)
        self.obstacle = self.node.new_subscription(PointCloud2,"carla/ego_vehicle/semantic_lidar",self.callback_,qos_profile=10)
        self.AEB_obstacle_publisher = self.node.new_publisher(Float32,"/AEB_Obstacle", qos_profile=fast_qos)
    ...
    def callback_(self,ros_point_cloud):
        xyz = np.array([[0,0,0]])
        rgb = np.array([[0,0,0]])
        #self.lock.acquire()
        gen = pc2.read_points(ros_point_cloud, skip_nans=True)
        int_data = list(gen)
        local_AEB_activated = False
        distance_min = list(int_data[0])
        distance_min[0] = max(distance_min[0],100)
        for x in int_data:
            # prints r,g,b values in the 0-255 range
                        # x,y,z can be retrieved from the x[0],x[1],x[2]
            if (distance_min[0] > 6.5 and x[0]> 6.5 and x[1] < 0.7 and x[1]> -0.7 and x[2] < 1 and x[2]> -1.3):
                distance_min[0] = min(distance_min[0],x[0])
            if(x[0] < 6.5 and x[0] > 0.5 and x[1] < 0.7 and x[1]> -0.7 and x[2] < 1 and x[2]> -1.3):
                xyz = np.append(xyz,[[x[0],x[1],x[2]]], axis = 0)
                if self._control.reverse == False:
                    self.node.logwarn("x0--------------------------------: {}".format(x[0]- 2.3))
                    #self.node.logwarn("x1--------------------------------: {}".format(x[1]))
                    #self.node.logwarn("x2--------------------------------: {}".format(x[2]))
                    #self._control.brake = 1
                    self.AEB_obstacle_publisher.publish(x[0]-2.3)
                    self.AEB_activated = True
                    local_AEB_activated = True
                    break
            
        if(local_AEB_activated == False): 
            self.AEB_obstacle_publisher.publish(distance_min[0])
                #print('self._control.brake = 1.0-------')
        if local_AEB_activated == False:
            #self._control.brake = 0
            self.AEB_activated = False
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        ...
        self._control.brake = jsInputs[4]
        if (keys[K_DOWN] or keys[K_s]):
            self._control.brake = 1.0 
        elif (self.AEB_activated == True and self._control.reverse == False):
            self._control.brake = 1.0 
        
        if self._control.brake  < 0.2:
            self._control.brake = 0
        self._control.hand_brake = bool(keys[K_SPACE])
        self._control.steer = min(0.5, max(-0.5, (jsInputs[0]/5.0)))
        #self._control.throttle = (((jsInputs[1] - (-1)) * (1.0 - 0)) / (1.0 - (-1.0))) - 0
        #self._control.brake = (((jsInputs[4] - (-1)) * (1.0 - 0)) / (1.0 - (-1.0))) - 0.5
        self._control.throttle = (((jsInputs[1]) * (1.0 - 0)) / (2.0 - (-1.0))) - 0
