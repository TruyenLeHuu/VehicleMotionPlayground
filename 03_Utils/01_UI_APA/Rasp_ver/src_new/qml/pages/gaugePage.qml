...
Item {
	...
	Rectangle {
		...
		property int lineW: mainWindow.is_small_screen*10 + mainWindow.is_big_screen*20  
		property int lineH: mainWindow.is_small_screen*50 + mainWindow.is_big_screen*100  
		property int distance:  mainWindow.is_small_screen*150 + mainWindow.is_big_screen*300  
		property string labelAnnouncement: ""
		...
		Rectangle {
			...
			z: 1
			x: (parent.width-width) /2 - 90*mainWindow.scale
			color: "white"
			SequentialAnimation {
				id: animationLine
				loops: 1
				...
				NumberAnimation { target: line0; property: "y"; from: -200*mainWindow.scale; to: 300*mainWindow.scale; duration: 4000}
			}
		}
		SequentialAnimation {
			...
			loops: 1
			running: false
			NumberAnimation { target: rectangle0; property: "carY"; from: rectangle0.carY; to: rectangle0.carY-220  ; duration: 1500}
			// NumberAnimation { target: rectangle0; property: "carSpeed"; from: mainWindow.is_small_screen*7 + mainWindow.is_big_screen*40  ; to: 0; duration: 500}
		}
		Rectangle {
				id: linet4
				height: rectangle0.lineH
		...
		Rectangle {
			...
			visible:true
			Announcement{
				x:mainWindow.is_small_screen*100 + mainWindow.is_big_screen*220
				y:0
			}
    	}
		Image 
	...
	Rectangle {
        ...
        anchors.top : parent.top
        anchors.topMargin: mainWindow.is_small_screen*340 + mainWindow.is_big_screen*640  
        anchors.left : parent.left
        anchors.leftMargin: (parent.width-height)/2
        visible: true
        color: "#00000000"
		CircularGauge {
			...
			style: CircularGaugeStyle {
				...
				tickmark: Rectangle {
					visible: false //|| styleData.value % 20 == 0  // styleData.value < 3 || 
					implicitWidth: outerRadius * 0.03
					antialiasing: true
				...
				minorTickmark: Rectangle {
					visible: false //styleData.value < 20  //|| styleData.value % 1 == 0
	...
	Rectangle {
        ...
        anchors.top : parent.top
        anchors.topMargin: mainWindow.is_small_screen*200 + mainWindow.is_big_screen*340  
        anchors.left : parent.left
        anchors.leftMargin: (parent.width-height)/2
        visible: true
        color: "#00000000"
	...
	Button {
		id: button3
		text: qsTr("Return")
		x: rectangle0.width - 120*mainWindow.car_scale
		y: rectangle0.height - 50*mainWindow.car_scale
		// enabled: true
		font.pointSize: 10*mainWindow.car_scale
		onClicked: {
			...
			rectangle0.carX = rectangle0.width/2;
			rectangle0.carY = rectangle0.height/2;
			rectangle0.labelAnnouncement = "";
			rectangle0.carRotation = 0;
		}
		...
		contentItem: Text {
			...
			font: button3.font
			opacity: enabled ? 1.0 : 0.3
			color: button3.down ? "#17a81a" : "#21be2b"
			horizontalAlignment: Text.AlignHCenter
			verticalAlignment: Text.AlignVCenter
			elide: Text.ElideRight
		}
		...
		background: Rectangle {
			...
			color: "#000000"
			opacity: enabled ? 1 : 0.3
			border.color: button3.down ? "#17a81a" : "#21be2b"
			border.width: 1
			radius: 2
		}
	}
	...
	Timer{
		...
		running: true
		onTriggered: {
			// console.log(backend.get_createCarParkingSlot())
			brakeDesireGauge.value = backend.get_desireBrakePressure();
			brakeGauge.value = backend.get_brakePressure();
			speedGauge.value = backend.get_vehicleSpeed();
	...
	Timer{
		...
		running: true
		onTriggered: {
			if (backend.get_doneCarParking() == 1) {
				console.log("done car parking")
				frontend.onDoneCarParking();
				backend.setDoneCarParking(0);
			}
			if (backend.get_createCarParkingSlot() == 1) {
				backend.setCreateCarParkingSlot(0);
				frontend.onCreateParkingSlot(backend.get_slots());
			}
			if (backend.get_isCommand() == 1)
			{
				frontend.onCreateCommand(backend.get_command())
				backend.setIsCommand(0)
			}
			// if (backend.get_isParking() == 1)
			// {
			// 	frontend.setTrueNeedle();
			// }
			// else {
			// 	frontend.setFalseNeedle();
			// 				}
		}
	}
	...
	Connections{
        ...
        // }
		// function setTrueNeedle()
		// {
		// 	brakeDesireNeedle.visible = true
		// }
		// function setFalseNeedle()
		// {
		// 	brakeDesireNeedle.visible = false
		// }
		function onCreateCommand(command)
		{
			rectangle0.labelAnnouncement = command
		}
		function onCreateParkingSlot(parkingSlots)
		{
			// console.log(parkingSlots[0].x);
            ...
            if (parkingSlotComponent.status === Component.Ready) {
				for (var i = 0; i < parkingSlots.length - 1; ++i) {
					const x_calculated = rectangle0.carX + (parkingSlots[parkingSlots.length - 1].y - parkingSlots[i].y) * 55
					const y_calculated = rectangle0.carY + 190 + (parkingSlots[parkingSlots.length - 1].x - parkingSlots[i].x) * 55
					console.log(rectangle0.carX)
					console.log(rectangle0.carY)
					console.log("car x " + car.x)
					console.log("car y " + car.y)
					console.log(parkingSlots[parkingSlots.length - 1].y)
					console.log(i + " " + parkingSlots[i].y)
					console.log(parkingSlots[parkingSlots.length - 1].x)
					console.log(i + " " + parkingSlots[i].x)
					console.log(i + " " + x_calculated)
					console.log(i + " " + y_calculated)
					const UIx = x_calculated - PARKING_SLOT_HEIGHT / 2;
					const UIy = y_calculated - PARKING_SLOT_WIDTH / 2;
					// const UIx = parkingSlots[i].x - PARKING_SLOT_HEIGHT / 2;
					// const UIy = parkingSlots[i].y - PARKING_SLOT_WIDTH / 2;
					if (y_calculated <1250 && y_calculated > 560){
						parkingSlots[i].id = parkingSlotComponent.createObject(	container, 
																				{slotNumber: i,
																				x: UIx,
																				y: UIy - 200 * mainWindow.scale - (car.height/2),
																				rotation: (x_calculated > rectangle0.carX ? 180 : 0)});
					}
					// console.log(parkingSlots[i].id)
				...
				// 	var labelAnnouncement = label.createObject(container, {x:mainWindow.is_small_screen*100 + mainWindow.is_big_screen*170, y:0});
				// }
        	}
		}
		function onDoneCarParking()
		{
			const PARKING_SLOT_WIDTH = 150*mainWindow.car_scale;
			const PARKING_SLOT_HEIGHT = 250*mainWindow.car_scale;
			...
			if (doneParkingSlotComponent.status === Component.Ready) {
				...
				initLocationCarAnimation.running = false;
			}
		}
