...
Item {
	...
	Button {
		...
		Button {
			...
			// enabled: true
			font.pointSize: 10*mainWindow.car_scale
			onClicked: frontend.onDoneCarParking()
		...
		function onCreateCarParkingSlot(parkingSlots)
		{
            ...
            if (parkingSlotComponent.status === Component.Ready) {
				for (var i = 0; i < parkingSlots.length - 1; ++i) {
					const x_calculated = rectangle0.carX + (parkingSlots[parkingSlots.length - 1].y - parkingSlots[i].y) * 40
					const y_calculated = rectangle0.carY + (parkingSlots[parkingSlots.length - 1].x - parkingSlots[i].x) * 40
				...
				if (label.status === Component.Ready) {
					var labelAnnouncement = label.createObject(container, {x:170, y:0});
				}
        	}
		}
